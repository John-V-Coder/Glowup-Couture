...
import { mergeGuestCart } from "../shop/cart-slice" // Updated path
...
export const loginUser = createAsyncThunk("auth/login", async (formData, { dispatch, rejectWithValue }) => {
  try {
    const response = await axios.post(
...
export const checkAuth = createAsyncThunk("auth/checkAuth", async (_, { rejectWithValue }) => {
  try {
    ...
    const message =
      error.response?.data?.message || error.message || "Authentication check failed. Please log in again."
    return rejectWithValue({ message })
  }
})
...
export const requestPasswordReset = createAsyncThunk(
  ...
  async (email, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/auth/request-password-reset`, { email })
      return response.data
    } catch (error) {
      const message =
        error.response?.data?.message || error.message || "Could not connect to the server. Please try again later."
      return rejectWithValue({ message })
    }
  },
)
...
export const verifyResetCode = createAsyncThunk(
  ...
  async ({ email, code }, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/auth/verify-reset-code`, { email, code })
      return response.data // This response should contain a temporary resetToken
    } catch (error) {
      const message = error.response?.data?.message || error.message || "Code verification failed. Please try again."
      return rejectWithValue({ message })
    }
  },
)
